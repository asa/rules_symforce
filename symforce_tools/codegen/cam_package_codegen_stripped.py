...
def generate(
    ...
    if isinstance(config, PythonConfig):
        ...
        #(asa) this init only should be run once. it ends up in sym/__init__.py
        #(asa) and loads all the camera and geo types
        #(asa) removed the one from geo_package_codegen_stripped.py
        # Package init
        ...
        templates.add(
            ...
            data=dict(
                Codegen.common_data(),
                all_types=list(GEO_TYPES) + list(CAM_TYPES),
